#!/bin/bash
# Build Java Language Specification ebook (MOBI + EPUB)
...
TITLE="Java SE 25 Language Specification"
AUTHOR="Oracle"

echo "📚 Building Java Language Specification ebook..."
echo "   Output directory: $OUTPUT_DIR"
echo ""

docker run --rm \
    -v "$SCRIPT_DIR/download_jls.py:/app/download.py:ro" \
    -v "$OUTPUT_DIR:/output" \
    ubuntu:22.04 bash -c "
set -e
...
ebook-convert /tmp/jls-se25.html /output/jls-se25.mobi \
    --title '$TITLE' \
    --authors '$AUTHOR' \
...
ebook-convert /tmp/jls-se25.html /output/jls-se25.epub \
    --title '$TITLE' \
    --authors '$AUTHOR' \
    --chapter '//h:h1[@class=\"title\"]' \
    --level1-toc '//h:h1[@class=\"title\"]' \
